<template>
  <div class="note-sidebar">
    <el-button size="mini" round class="add-note" @click="onAddNote">
      添加笔记
    </el-button>
    <el-dropdown
...
export default {
    ...
    this.getNotebooks()
      .then(_ => {
        this.setCurrentBookId({
          currentNotebookId: this.$route.query.notebookId
        })
  ...
  methods: {
    ...
    handleCommand(notebookId) {
      ...
      this.getNotes({ notebookId }).then(_ => {
    ...
    onAddNote() {
      this.addNote({ notebookId: this.currentNotebook.id }).then(noteId => {
